...
load("C:/Users/msses/Desktop/TCC/dados/ETF/etf_complete_effr.RData")
...
load( file = "C:/Users/msses/Desktop/TCC/dados/HF/hf_complete_effr.RData")
...
foreach(t = 1:1000,.packages = c('dplyr', 'tidyr', 'glmnet', 'lubridate','PerformanceAnalytics','moments')) %dopar% {
  ...
  result <- foreach(i = 2:100,.packages = c('dplyr', 'tidyr', 'glmnet', 'lubridate'), .combine = 'rbind') %dopar% {
    ...
    etf <- sweep(etf, 1, weights$weights, `*`)
  ...
  # Supondo que seus dataframes são etf e weights
  etf_lasso <- sweep(etf_lasso, 1, weights$weights, `*`)
  ...
  # Supondo que seus dataframes são etf e weights
  etf_adalasso <- sweep(etf_adalasso, 1, weights$weights, `*`)
  ...
  save(model_results, file = file.path("C:/Users/msses/Desktop/TCC/dados/result_effr",
                                       paste0("result_ewma_effr_test", unique(model_results$fund), unique(model_results$start_prediction), ".RData")
  ))
